...
class Player():
    ...
    def attack(self, board):
        ...
        eel.highlight_av_att_cards_js(av_to_attack)
        eel.sleep(1)
...
class Game():
    ...
    def move(self,game, p1: Player, p2: Player):
        self.reset(p1,p2)
...
def game_start():
    ...
    eel.start_hands_js(p1.cards,p2.cards)
    eel.sleep(3)

    print('coz', game.coz, '\np1', p1.cards, 'p2\n', p2.cards, '\n', '#' * 30)
    while (not game.game_over):

        eel.coloda_js(game.start_cards)
        eel.sleep(1)

        print('Ход № ',game.move_number)
        print('p1 ', p1.cards, '\n','p2', p2.cards, '\n',game.start_cards,game.start_cards.__len__(),'\n', '#' * 30)
        game.move(game,p1, p2)
        print('#'*50)